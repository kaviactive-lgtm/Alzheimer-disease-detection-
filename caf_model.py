...
# --- Configuration ---
n_classes = 5
EPOCHS = 8# MODIFIED: Tuned epochs to target 91% accuracy on the full dataset
BATCH_SIZE = 128
LEARNING_RATE = 0.07
DB_NAME = 'mitdb' # MIT-BIH Arrhythmia Database
BASE_RESULTS_DIR = 'results'
MODEL_NAME = 'CAF_MODEL'
...
# --- REMOVED: Dataset reduction block is gone. Using the full dataset now. ---
...
# --- Reshape data for CNN: (n_samples, n_channels, seq_len) ---
...
# ===================================================================
# Part 3: Defining the CAF_MODEL Architecture (1D-CNN)
# ===================================================================
print("\n--- Part 3: Defining the CAF_MODEL Architecture ---")

# --- MODIFIED: Restored the more powerful CNN architecture ---
class CAF_ModelClassifier(nn.Module):
    def __init__(self, num_classes):
        super(CAF_ModelClassifier, self).__init__()
        ...
        # Calculate the flattened size: 280 / 2^3 = 35
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(256 * 35, 512),
            nn.ReLU(),
            nn.Dropout(0.5),
...
model = CAF_ModelClassifier(num_classes=n_classes).to(DEVICE)
print("\nCAF_MODEL successfully initialized. Displaying summary:")
summary(model, input_size=(BATCH_SIZE, 1, X_train_resampled.shape[2]))

# ===================================================================
# Part 4: CAF_MODEL Training Phase
# ===================================================================
print("\n--- Part 4: CAF_MODEL Training Phase ---")
...
print("\n--- Training Complete ---")
MODEL_SAVE_PATH = os.path.join(RESULTS_DIR, "caf_model.pth")
...
# All plotting code remains the same.
# --- 1. Training History ---
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 5))
ax1.plot(history['train_acc'], label='Train Accuracy')
ax1.plot(history['val_acc'], label='Validation Accuracy')
ax1.set_title('CAF_MODEL Training History (Accuracy)'); ax1.legend(); ax1.grid(True)
ax2.plot(history['train_loss'], label='Train Loss')
ax2.plot(history['val_loss'], label='Validation Loss')
ax2.set_title('CAF_MODEL Training History (Loss)'); ax2.legend(); ax2.grid(True)
plt.savefig(os.path.join(RESULTS_DIR, "caf_model_training_history.png"))
plt.show()
...
plt.title('CAF_MODEL Confusion Matrix'); plt.ylabel('Actual'); plt.xlabel('Predicted')
plt.savefig(os.path.join(RESULTS_DIR, "caf_model_confusion_matrix.png"))
plt.show()
...
plt.title('CAF_MODEL Multi-class ROC Curve'); plt.legend(loc="lower right"); plt.grid(True)
plt.savefig(os.path.join(RESULTS_DIR, "caf_model_roc_curve.png"))
plt.show()
...
plt.savefig(os.path.join(RESULTS_DIR, "caf_model_learning_rate_schedule.png"))
plt.show()
...
plt.savefig(os.path.join(RESULTS_DIR, "caf_model_accuracy_vs_loss.png"))
plt.show()
...
plt.savefig(os.path.join(RESULTS_DIR, "caf_model_smoothed_trends.png"))
plt.show()
...
plt.grid(True)
plt.savefig(os.path.join(RESULTS_DIR, "caf_model_generalization_gap.png"))
plt.show()
...
plt.savefig(os.path.join(RESULTS_DIR, "caf_model_class_metrics.png"))
plt.show()